...
function AdminCreateLawyer() {
  ...
  useEffect(() => {
    ...
    if (!dni.trim()) {
      setResults([]);
      setMessage('Escriba un DNI para buscar usuarios');
      return;
    }
    setLoading(true);
    setMessage('');
    timerRef.current = setTimeout(async () => {
      try {
        ...
        if (normalized.length === 0) setMessage('No se encontraron usuarios con ese DNI');
      } catch {
        setResults([]);
    ...
    <div className="min-h-screen bg-[#ECEFF3] px-4 sm:px-6 py-6 animate-fade-in">
      <div className="max-w-6xl mx-auto w-full">
        <div className="flex items-center gap-3 mb-6 animate-slide-up">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-[#53667B]">Buscar usuario</h1>
        </div>

        <div className="bg-white/70 rounded-3xl p-6 sm:p-8 shadow-soft animate-slide-up space-y-4" style={{ animationDelay: '100ms' }}>
          <p className="text-center text-[#53667B] text-lg sm:text-xl font-semibold leading-relaxed mb-4">
          ...
          {results.length > 0 && (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {results.map(user => (
                <button
                  key={user.id}
                  onClick={() => navigate('/admin/lawyers/create/user-found', { state: { user } })}
